    ...
    <div class="container text-center px-3">
        ...
        <div class="row justify-content-center">
            <div class="col-md-6 col-lg-5">
                <form action="{{ route('produk') }}" method="GET" class="input-group shadow-sm">
                    <input type="text" name="search" class="form-control border-0"
                           placeholder="Cari keripik sanjai..." value="{{ request('search') }}">
                ...
                @if(request('search'))
                    <div class="mt-2 small text-muted">
                        ...
                        <a href="{{ route('produk') }}" class="text-decoration-none ms-2 text-danger">Hapus</a>
                    </div>
                @endif
            </div>
        </div>
    ...
    <div class="row g-3">
        ...
        @empty
            <div class="col-12 text-center py-5">
                ...
                <p class="text-muted">Produk "{{ request('search') }}" tidak ditemukan.</p>
                <a href="{{ route('produk') }}" class="btn btn-outline-warning btn-sm">Lihat Semua Produk</a>
            </div>
        @endforelse
    </div>
</div>
